...
本 Feature 提供 Python 最小可运行实现，默认 Connector 使用 Mock 数据，便于自检和回归。Mock 实现仍保留完整 Config、Input Schema、Output Schema、Error Handling、Retry Policy、Cache Strategy、Health Check 和 Test Case，以保证未来替换为真实 API 时接口稳定。
