...
QA-SPRINT-3 处理：新增 `scripts/validate_stable_release.py` 并纳入 `production_check.py`；Production E2E artifact 必须包含 `stable_release_gate`。旧脚本仍保留为 artifact checks，Stable gate 负责验证 mock/SKIP/fallback 不计入真实生产 PASS。
...
QA-SPRINT-3 处理：新增 `common/release_gate.py`，APP-001 输出 `stable_release_gate`；CLI `--real-data` 在 degraded source 存在时返回非 0；Connector real-mode fallback 统一标记为 `fallback_mock`。剩余工作是由人工在 Docker daemon 和真实外部源可用环境执行不可 SKIP 的 real-mode release case。
...
QA-SPRINT-3 处理：API 和 CLI 顶层输出暴露 `data_lineage` / `stable_release_gate`，避免调用方误把 degraded completion 当作 Stable success。完整 Core-driven Evidence/KG/Score builder 收敛仍未完成。
...
QA-SPRINT-2 处理：默认 localhost，非本地绑定必须设置 `AIRS_API_KEY`，新增 CORS allowlist、请求体限制和错误脱敏。

QA-SPRINT-3 处理：新增 API rate limit，invalid JSON 默认脱敏；Docker Compose 增加带 API key 的 `/health` healthcheck。剩余工作是在 Docker daemon 可用环境验证 image build / compose up / container health。
...
QA-SPRINT-3 处理：新增 `config/airs.stable.yaml`，并在 `config/README.md` 中声明 demo profile 与 Stable profile 的区别。完整 artifact source-of-truth manifest 尚未完成。
...
QA-SPRINT-3 处理：CLI/API/APP 输出对 `completed_with_degradation` 与 `failed_quality_gate` 的传播更明确；`--real-data` 失败时 CLI 返回非 0。
