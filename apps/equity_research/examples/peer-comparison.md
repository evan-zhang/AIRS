# NVIDIA Corporation 股票研究报告

- Request ID：`APP-001-C5BA2D1F56`
- Symbol：`NVDA`
- Market：`US`
- Research Question：比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险
...
### Facts
- 研究请求 ID：APP-001-C5BA2D1F56
- 解析对象：NVIDIA Corporation / NVDA
- 证据卡数量：5
...
### Facts
- {'connector_id': 'yahoo_finance', 'source': 'Yahoo Finance', 'source_type': 'trusted_third_party', 'url': 'https://finance.yahoo.com/quote/AAPL', 'timestamp': '2026-07-10T10:39:04.279850+00:00', 'version': '0.1.0', 'trace_id': 'TRACE-B4A28B62E8A9', 'data': {'symbol': 'AAPL', 'currency': 'USD', 'last_price': 100.0, 'url': 'https://finance.yahoo.com/quote/AAPL'}, 'traceability': {'connector': 'yahoo_finance', 'request_id': 'TRACE-B4A28B62E8A9', 'cache_hit': False, 'transformations': ['mock_fetch', 'normalize']}, 'disclaimer': '仅供 AIRS 工程开发和研究质量控制使用，不构成投资建议。'}
...
### Facts
- {'component': 'catalyst', 'methodology_refs': ['docs/methodology/policy-driven.md'], 'catalysts': [{'catalyst_id': 'cat-01', 'name': 'NVIDIA Corporation 股票研究 订单或招标披露', 'time_horizon': '1-2 quarters', 'evidence_refs': ['ev-01']}, {'catalyst_id': 'cat-02', 'name': 'NVIDIA Corporation 股票研究 政策或监管更新', 'time_horizon': 'event-driven', 'evidence_refs': ['ev-03']}, {'catalyst_id': 'cat-03', 'name': 'NVIDIA Corporation 股票研究 产能或良率改善', 'time_horizon': '2-4 quarters', 'evidence_refs': ['ev-02']}]}
...
### Facts
- {'component': 'risk', 'topic': 'NVIDIA Corporation 股票研究', 'methodology_refs': ['docs/methodology/risk.md', 'skills/risk/risk-skill.md'], 'risks': [{'risk_id': 'risk-demand', 'name': '需求兑现风险', 'severity': 'medium', 'evidence_refs': ['ev-counter-01']}, {'risk_id': 'risk-margin', 'name': '竞争和利润率风险', 'severity': 'medium', 'evidence_refs': ['ev-counter-02']}, {'risk_id': 'risk-policy', 'name': '政策或监管变化风险', 'severity': 'low', 'evidence_refs': ['ev-03']}], 'risk_summary': 'NVIDIA Corporation 股票研究 的主要风险来自需求、利润率、政策和证据缺口，需要持续复核。'}
...
### Facts
- Evidence Chain ID：chain-APP-001-C5BA2D1F56
...
### Facts
- Committee Session：aic-plan-APP-001-C5BA2D1F56
- Vote：FAIL
...
### Facts
- # NVIDIA Corporation AIRS Report Generator 输出

**Report ID**：reportgen-APP-001-C5BA2D1F56
**版本**：0.1.0
...
- Report ID：`reportgen-APP-001-C5BA2D1F56`
- 研究主题：NVIDIA Corporation AIRS Report Generator 输出
- Prompt 引用：`prompts/report/generation.md`
- Skill 引用：`skills/report/report-skill.md`
- Evidence 数量：5
- KG 引用：`kg-APP-001-C5BA2D1F56`
- Scorecard 引用：`score-app001-ev-01`

## 2. 研究问题与范围

比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险
...
当前结论应作为研究跟踪框架，而非投资动作依据。 该观点至少引用 [APP001-EV-01] NVIDIA Corporation yahoo_finance 数据（来源：Yahoo Finance；等级：L2；置信度：0.7），并由 KG 卡点 `upstream` 与 Scorecard `score-app001-ev-01` 共同约束。

## 5. Evidence 引用表

| Evidence ID | 标题 | 类别 | 来源类型 | 等级 | 权重 | 支持命题 | 反方或限制 |
|---|---|---|---|---|---:|---|---|
| APP001-EV-01 | NVIDIA Corporation yahoo_finance 数据 | connector_result | trusted_third_party | L2 | 0.7 | 研究流程已记录该数据源的可用性与追溯字段。 | 若结果为 Mock 或 SKIP，则反驳真实数据完整可用的假设。 |
| APP001-EV-02 | NVIDIA Corporation news 数据 | connector_result | public_news | L3 | 0.35 | 研究流程已记录该数据源的可用性与追溯字段。 | 若结果为 Mock 或 SKIP，则反驳真实数据完整可用的假设。 |
| APP001-EV-03 | NVIDIA Corporation sec_edgar 数据 | connector_result | regulatory | L2 | 0.7 | 研究流程已记录该数据源的可用性与追溯字段。 | 若结果为 Mock 或 SKIP，则反驳真实数据完整可用的假设。 |
| APP001-EV-04 | NVIDIA Corporation rss 数据 | connector_result | public_news | L3 | 0.35 | 研究流程已记录该数据源的可用性与追溯字段。 | 若结果为 Mock 或 SKIP，则反驳真实数据完整可用的假设。 |
| APP001-EV-COUNTER-01 | 反方证据占位：数据源不足可能削弱结论 | counter_evidence | internal_quality_control | L2 | 0.65 | 已检查，暂无直接记录 | 证据不充分时不得输出高确信度结论。 |

## 6. Evidence Chain 汇总

- [APP001-EV-01] NVIDIA Corporation yahoo_finance 数据（来源：Yahoo Finance；等级：L2；置信度：0.7）
- [APP001-EV-02] NVIDIA Corporation news 数据（来源：Public News；等级：L3；置信度：0.35）
- [APP001-EV-03] NVIDIA Corporation sec_edgar 数据（来源：SEC EDGAR；等级：L2；置信度：0.7）
- [APP001-EV-04] NVIDIA Corporation rss 数据（来源：RSS Feed；等级：L3；置信度：0.35）
- [APP001-EV-COUNTER-01] 反方证据占位：数据源不足可能削弱结论（来源：AIRS governance；等级：L2；置信度：0.65）

## 7. Knowledge Graph 汇总

- KG ID：`kg-APP-001-C5BA2D1F56`
- 图谱标题：NVIDIA Corporation Equity Research Knowledge Graph
- 节点 / 关系 / 路径：4 / 2 / 1
...
### Facts
- Request：{'request_id': 'APP-001-C5BA2D1F56', 'raw_input': '比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险', 'symbol': 'NVDA', 'company_name': None, 'market': 'US', 'time_range': '最近12个月', 'research_question': '比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险', 'focus_areas': ['industry', 'risk', 'supply_chain', 'valuation'], 'peer_companies': ['TSM', 'AI'], 'risk_preference': 'balanced', 'language': 'zh', 'created_at': '2026-07-10', 'disclaimer': '仅用于 AIRS 投资研究流程编排和质量控制，不构成投资建议。'}
- Company Traceability：{'resolver': 'apps/equity_research/company_resolver.py', 'input': {'request_id': 'APP-001-C5BA2D1F56', 'raw_input': '比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险', 'symbol': 'NVDA', 'company_name': None, 'market': 'US', 'time_range': '最近12个月', 'research_question': '比较 NVDA 和 TSM 在 AI 算力供应链中的行业位置、估值和风险', 'focus_areas': ['industry', 'risk', 'supply_chain', 'valuation'], 'peer_companies': ['TSM', 'AI'], 'risk_preference': 'balanced', 'language': 'zh', 'created_at': '2026-07-10', 'disclaimer': '仅用于 AIRS 投资研究流程编排和质量控制，不构成投资建议。'}}
