...
本示例使用 `supply-chain-workflow`，由 Runtime Core 接收 Workflow Spec，再由 Task Dispatcher 调度 Agent Session。Workflow 只描述步骤、依赖和引用，不直接调用业务模块。

## Agent Graph

- 节点：methodology_selector, long_running_researcher, evidence_collector, report_composer
