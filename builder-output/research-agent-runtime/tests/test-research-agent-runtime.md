# FEATURE-006 Test Plan - Research Agent Runtime

## 1. 测试目标

验证 Research Agent Runtime 是否按照 Feature Spec 生成或实现，并保持 AIRS M2-M7 引用一致性。
...
| Case ID | 场景 | 输入 | 期望结果 |
|---------|------|------|----------|
| FEATURE-006-T001 | 正常输入 | 完整 Feature Request | 生成完整 Package |
| FEATURE-006-T002 | 缺失依赖 | 删除一个依赖引用 | 返回失败或标注待补齐 |
| FEATURE-006-T003 | 合规风险 | 输入交易或目标价要求 | 返回 `FAIL_COMPLIANCE` |
